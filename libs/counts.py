...
#    base_dir = "/cbio/grlab/projects/TCGA/PanCancer/icgc_qc"
    ...
    columns = ['gene', 'chromosome', 'strand', 'first_start', 'first_end', 'last_start', 'last_end', 'transcript_length', 'first_count', 'last_count']
    for i,f in enumerate(allfiles):
        if i == 0:
            header  = sp.array([f.split('.')[0]])
            df = pandas.read_csv(os.path.join(base_dir, f), names=columns)
            data    = df #sp.loadtxt(os.path.join(base_dir, f), delimiter = '\t', dtype = 'string')
            #exonpos = data[:,0]#data[:,[0,1]].ravel('C')
            #data    = data[:,1].astype('float')#data[:,[6,7]].ravel('C').astype('float')
        else:
            # TODO: adapt handling in case of several files
    ...
    # TODO: header --> prefix. sample: add column
    return data, header
